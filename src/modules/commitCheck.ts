...
function parseStatusLine(line: string): { status: string; path: string } | null {
...
function isModifiedOrNew(status: string): boolean {
    if (status === '??') return true;
    if (/M|A/.test(status)) return true;
    return false;
}

/** 在指定工作区目录执行 git 并返回分支名与修改/新增文件列表 */
export function fetchGitStatus(cwd: string): GitStatusResult {
...
function buildFilesHtml(fileItems: FileItem[]): string {
    if (fileItems.length === 0) return '<p class="empty">无修改或新增文件</p>';
    return fileItems
        .map(
            (f) =>
...
function buildReviewPrompt(fileItems: FileItem[]): string {
...
function runCursorAgentPrint(prompt: string, cwd: string): Promise<string> {
...
export function run(): void {
    ...
    const panel = vscode.window.createWebviewPanel(
        'aicode-tools.commitCheck',
        `提交检测 - ${data.branchName}`,
        vscode.ViewColumn.One,
        { enableScripts: true }
    );

    function updateWebview(result: GitStatusResult) {
    ...
    panel.webview.onDidReceiveMessage(async (msg: { type: string }) => {
        if (msg.type === 'refresh') {
            ...
            try {
                ...
                const isNoCli = message.includes('未找到 Cursor Agent CLI') || (err as NodeJS.ErrnoException & { code?: string })?.code === 'ENOENT';
                if (isNoCli) {
...
function getWebviewContent(branchName: string, filesHtml: string, hasList: boolean): string {
    return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
